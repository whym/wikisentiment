...
class SentiWordNetExtractor:
    ...
    def extract(self, entry, threshold=0.1):
        ...
        for w in words:
            if self.avg_scores.has_key(w):
                if abs(self.avg_scores[w]) > threshold:
                    ret[w] = self.avg_scores[w]
        return ret
    def name(self):
        return 'SentiWordNetExtractor'

if __name__ == '__main__':
    # some examples
    fx = SentiWordNetExtractor('SentiWordNet_3.0.0_20100908.txt')
